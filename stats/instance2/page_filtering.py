...
def search_for_page():
    ...
    if ("keywords",) in mynames:
        ...
        #insert in one go:
        args_str = ','.join(['%s'] * len(keys.kwd))
        myquery = 'INSERT INTO keywords(kwd) VALUES {0}'.format(args_str)
        cursor.execute(myquery, [(k,) for k in keys.kwd] )
        conn.commit()
    ...
    #Test if there is something to insert:
    if X!=[]:
        #insert into link column all links  one go:
        args_str = ','.join(['%s'] * len(X))
        myquery = 'INSERT INTO potential_pages(link) VALUES {0}'.format(args_str)
        cursor.execute(myquery, [(k,) for k in X] )
        conn.commit()
    #print information for the end user so that they know how many new links
    #has been crawled:
